...
home_page_html = '''<style>

</style>
<div class="page_html">
 <header class="hi"><span class="color_b">Home</span></header>
...
<div class="page_html">
 <header class="hi"><span class="color_b">Blog</span></header>

</div><!-- - /page_html - -->'''

team_page_html = '''<style>

</style>
<div class="page_html">
 <header class="hi"><span class="color_b">Team</span></header>

</div><!-- - /page_html - -->'''

resources_page_html = '''<style>

</style>
<div class="page_html">
 <header class="hi"><span class="color_b">Resources</span></header>
...
input[type="message"] { width: 200px; height: 150px; }
</style>
<div class="page_html">
 <header class="hi"><span class="color_b">Contact</span></header>
    ...
    <form action="../../add_form" enctype="multipart/form-data" method="post">
      <table>
        ...
        <tr>
          ...
          <td class="input"><textarea cols="40" rows="4" type="text" name="message" /></textarea></td>
        </tr>
        <tr>
          <td></td>
  ...
  </article><!-- - /contact_wrap - -->
</div><!-- - /page_html - -->'''

schedule_page_html = '''<style>

</style>
<div class="page_html">
 <header class="hi"><span class="color_b">Schedule</span></header>
  ...
  <article class="list_wrap">
    <div class="item_wrap" ng-repeat="item in items">
      <div class="item_data">
        ...
        <br />
        <span class="user_name">[!item.user_name!]</span><br />

        <span class="message">[!item.message!]</span>/p>
...
class addForm_db(webapp2.RequestHandler):
    def post(self):
      ...
      # - -
        ...
        item.message = self.request.get('message')
      #
        item.put()
        time.sleep(1)
    ...
    def get(self):
      ...
      # - app data
        ...
        if path_layer == 'home':
            page_id = 'home'
            page_name = 'Home'
            page_html = home_page_html

        if path_layer == 'blog':
            page_id = 'blog'
            page_name = 'Blog'
            page_html = blog_page_html

        if path_layer == 'team':
            page_id = 'team'
            page_name = 'Team'
            page_html = team_page_html

        if path_layer == 'resources':
            page_id = 'resources'
            page_name = 'Resources'
            page_html = resources_page_html

        if path_layer == 'contact':
            page_id = 'contact'
            page_name = 'Contact'
            page_html = contact_page_html

        if path_layer == 'schedule':
            page_id = 'schedule'
            page_name = 'Schedule'
            page_html = schedule_page_html
...
class listData(webapp2.RequestHandler):
    def get(self):
        ...
        if users.is_current_user_admin():
            if base == 'entries':
                q = db.Query(Form_db, projection=('data_id', 'email_address', 'user_name', 'message'))
